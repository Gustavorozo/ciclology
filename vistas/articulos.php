...
if(isset($_SESSION['usuario'])){
	...
	<head>
		...
		from categorias";
        $sql="SELECT id_marca,nombreMarca
        from Marca";
        $sql="SELECT id_color,nombreColor
		from Color";
				...
				<div class="col-sm-4">
					<form id="frmArticulos" enctype="multipart/form-data">
                        ...
                        <select class="form-control input-sm" id="MarcaSelect" name="MarcaSelect">
                            ...
                            <?php while($ver=mysqli_fetch_row($result)): ?>
                                <option value="<?php echo $ver[3] ?>"><?php echo $ver[4]; ?></option>
                            <?php endwhile; ?>
                        </select>
                        <label>Color</label>
                        <select class="form-control input-sm" id="ColorSelect" name="ColorSelect">
                            ...
                            <?php while($ver=mysqli_fetch_row($result)): ?>
                                <option value="<?php echo $ver[4] ?>"><?php echo $ver[5]; ?></option>
                            <?php endwhile; ?>
                        </select>
						<label>Nombre</label>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form id="frmArticulosU" enctype="multipart/form-data">
                            ...
                            <label>Marca</label>
                            <select class="form-control input-sm" id="MarcaSelectU" name="MarcaSelectU">
                                <option value="A">Selecciona Marca</option>
                                <?php
                                $sql="SELECT id_marca,nombreMarca
								from Marca";
		...
		function agregaDatosArticulo(idarticulo){
			$.ajax({
				...
				success:function(r){
                    ...
                    $('#MarcaSelectU').val(dato['id_marca']);
                    $('#ColorSelectU').val(dato['id_color']);
					$('#NombreU').val(dato['Nombre']);
					$('#descripcionU').val(dato['Talla']);
